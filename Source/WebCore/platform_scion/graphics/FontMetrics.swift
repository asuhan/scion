...
class FontMetricsWrapper {
  ...
  func intHeight(baselineType: FontBaseline = .AlphabeticBaseline) -> Int32 {
    return wk_interop.FontMetrics_intHeight(p!, baselineType.rawValue)
  }
  ...
  func capHeight() -> Float32? {
    let capHeight = wk_interop.FontMetrics_capHeight(p!)
    if !capHeight.is_valid {
      return nil
    }
    return capHeight.value
  }
