...
extern "C" WEBCORE_EXPORT bool RenderBlockFlow_containsFloats(const void* p)
{
    return static_cast<const WebCore::RenderBlockFlow*>(p)->containsFloats();
}
...
extern "C" WEBCORE_EXPORT uint32_t PaintInfo_paintBehavior(const void* p)
{
    const auto o = static_cast<const WebCore::PaintInfo*>(p)->paintBehavior;
    return o.toRaw();
}
...
bool RenderBlockFlow::hasInlineLayout() const
{
    if (m_scion) { return m_scion->hasInlineLayout(); }
...
{
    if (m_scion) {
        m_scion->marginBeforeEstimateForChild(child, positiveMarginBefore, negativeMarginBefore);
        return;
    }
