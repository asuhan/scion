...
extern "C" WEBCORE_EXPORT int32_t RenderBox_x(const void* p)
{
    const auto x = static_cast<const WebCore::RenderBox*>(p)->x();
    return x.rawValue();
}
...
extern "C" WEBCORE_EXPORT int32_t RenderBox_width(const void* p)
{
    const auto width = static_cast<const WebCore::RenderBox*>(p)->width();
...
extern "C" WEBCORE_EXPORT void* RenderBox_previousSiblingBox(const void* box_raw_ptr)
{
    return static_cast<const WebCore::RenderBox*>(box_raw_ptr)->previousSiblingBox();
}
...
extern "C" WEBCORE_EXPORT int32_t RenderBox_marginAfter(const void* box_raw_ptr, const void* override_style_raw_ptr)
{
    const auto override_style = static_cast<const WebCore::RenderStyle*>(override_style_raw_ptr);
    const auto width = static_cast<const WebCore::RenderBox*>(box_raw_ptr)->marginAfter(override_style);
    return width.rawValue();
}
...
extern "C" WEBCORE_EXPORT void RenderBox_addVisualOverflow(void* p, LayoutRectRaw rect_raw) {
    WebCore::LayoutRect rect(
        ...
        WebCore::LayoutUnit::fromRawValue(rect_raw.width),
        WebCore::LayoutUnit::fromRawValue(rect_raw.height));
    static_cast<WebCore::RenderBox*>(p)->addVisualOverflow(rect);
}
...
{
    if (m_scion) { return m_scion->marginAfter(overrideStyle); }
...
void RenderBox::setMarginAfter(LayoutUnit value, const RenderStyle* overrideStyle)
{
    if (m_scion) {
        m_scion->setMarginAfter(value, overrideStyle);
        return;
    }
...
{
    if (m_scion) { return m_scion->scrollsOverflowY(); }
...
{
    if (m_scion) { return m_scion->hitTestClipPath(hitTestLocation, accumulatedOffset); }
    if (!style().clipPath())
        return true;
...
LayoutRect RenderBox::overflowClipRect(const LayoutPoint& location, RenderFragmentContainer* fragment, OverlayScrollbarSizeRelevancy relevancy, PaintPhase phase) const
{
    if (m_scion) { return m_scion->overflowClipRect(location, fragment, relevancy, phase); }
...
{
    if (m_scion) { return m_scion->isUnsplittableForPagination(); }
