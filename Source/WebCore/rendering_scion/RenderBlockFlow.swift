...
class RenderBlockFlowWrapper: RenderBlockWrapper {
  ...
  struct MarginInfo {
    ...
    func canCollapseWithMarginBefore() -> Bool {
      return atBeforeSideOfBlock && m_canCollapseMarginBeforeWithChildren
    }
  ...
  func shouldBreakAtLineToAvoidWidow() -> Bool {
    assert(isNativeImpl())
    return hasRareBlockFlowData() && rareBlockFlowData().lineBreakToAvoidWidow >= 0
  }
  ...
  func lineBreakToAvoidWidow() -> Int32 {
    assert(isNativeImpl())
    return hasRareBlockFlowData() ? rareBlockFlowData().lineBreakToAvoidWidow : -1
  }
  ...
  func clearDidBreakAtLineToAvoidWidow() {
    ...
    rareBlockFlowData().didBreakAtLineToAvoidWidow = false
  }
  ...
  func logicalLeftForFloat(_ floatingObject: FloatingObjectWrapper) -> LayoutUnit {
    assert(isNativeImpl())
    return isHorizontalWritingMode() ? floatingObject.x() : floatingObject.y()
  }

  func logicalRightForFloat(_ floatingObject: FloatingObjectWrapper) -> LayoutUnit {
    assert(isNativeImpl())
  ...
  func lineLayoutPath() -> LineLayoutPath {
    assert(isNativeImpl())
    return renderBlockFlowLineLayoutPath()
  }
