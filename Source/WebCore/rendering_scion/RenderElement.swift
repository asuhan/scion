...
class RenderElementWrapper: RenderObjectWrapper {
  init(
    ...
    self.style = style
    super.init(type, element, baseTypeFlags, typeSpecificFlags)
  ...
  func lastChild() -> RenderObjectWrapper? {
    if !isNativeImpl() {
      if let childRaw = wk_interop.RenderElement_lastChild(id()) {
        return createRenderObjectWrapper(childRaw)
      }
      return nil
    }
    return m_lastChild
  ...
  func shouldApplyInlineSizeContainment() -> Bool {
    assert(isNativeImpl())
    return layout_scion.isSkippedContentRoot(style: style(), element: element())
      || shouldApplySizeOrStyleContainment(style().containsInlineSize())
  }
  ...
  func hasContinuationChainNode() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderElement_hasContinuationChainNode(id()) }
