...
class RenderObjectWrapper: CachedImageClientWrapper {
  ...
  func enclosingFragmentedFlow() -> RenderFragmentedFlowWrapper? {
    if !isNativeImpl() {
      guard let fragmentedFlowRaw = wk_interop.RenderObject_enclosingFragmentedFlow(id()) else {
        return nil
      }
      return createRenderObjectWrapperOrNative(fragmentedFlowRaw) as? RenderFragmentedFlowWrapper
    }
  ...
  func locateEnclosingFragmentedFlow() -> RenderFragmentedFlowWrapper? {
    assert(isNativeImpl())
    return containingBlock()?.enclosingFragmentedFlow()
  }
  ...
  func isRenderInline() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderObject_isRenderInline(id()) }
  ...
  func isRenderTextFragment() -> Bool {
    assert(isNativeImpl())
  ...
  func isRenderOrLegacyRenderSVGRoot() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderObject_isRenderOrLegacyRenderSVGRoot(id()) }
  ...
  func isBRForRenderObject() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderObject_isBR(id()) }
  ...
  func isLineBreakOpportunity() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderObject_isLineBreakOpportunity(id()) }
  ...
  func isInline() -> Bool {
    if !isNativeImpl() {
      return wk_interop.RenderObject_isInline(id())
    }
  ...
  func needsSimplifiedNormalFlowLayout() -> Bool {
    if !isNativeImpl() { return wk_interop.RenderObject_needsSimplifiedNormalFlowLayout(id()) }
  ...
  func page() -> PageWrapper {
    if !isNativeImpl() {
      return PageWrapper(wk_interop.RenderObject_page(id()))
    }
